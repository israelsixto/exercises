...
sorted_cars <- arrange(vehicles_1997, hwy)
...
two_wheel <- filter(vehicles, drive == "2-Wheel Drive") %>% # step 1
  filter(cty > 20)
...
worstest_vehicles <- select(worst_vehicle, id)
...
best_in_year <- function(year_choice, make_choice){
  car_makes_and_year <- filter(vehicles, make == make_choice) %>%
    filter(year == year_choice) %>%
    filter(hwy == max(hwy))
  
  best_car <- select(car_makes_and_year, model)
}

# What was the most efficient Honda model of 1995?

efficient_honda <- best_in_year(1995, "Honda")
